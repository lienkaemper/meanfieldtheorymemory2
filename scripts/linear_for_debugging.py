...
for i in range(trials):
    for k, h in enumerate(h_range):
        ...
        pred_rates = y_pred_full(J, b)
    
        pred_covs = covariance_full(J, pred_rates)
        pred_cors_mat = cov_to_cor(pred_covs)

        J_small = macro_weights(J = J0, h3 = h, h1 = h, g = g, g_ii = g_ii)
        pred_rates_small = y_pred_full(J, b)
    

        max_rate = np.max(pred_rates)
        maxspikes = int(np.floor(N*max_rate*tstop ))
        gc.collect()
        v, spktimes = sim_glm_pop(J=J,  E=b, dt = dt, tstop=tstop,  v_th = 0, maxspikes = maxspikes, p = 1)
