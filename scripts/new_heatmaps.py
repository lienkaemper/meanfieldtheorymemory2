...
for i, g in enumerate(gs): 
    for j, g_ii in enumerate(g_iis): 
        ...
        h_i = find_iso_rate(y_before[i,j], h_max, J0, g, g_ii, b, h_i_min, h_i_max, "quadratic")
        h_is[i,j] = h_i
        G_after_plast = macro_weights(J = J0, h1 = h_max, h3 = h_max ,g=g, h_i=h_i, g_ii=g_ii)
...
cs = axs[0].imshow(delta_cor, origin="lower", extent = (g_ii_min, g_ii_max, g_min, g_max), vmin = 1, vmax = 8)
plt.colorbar(cs, ax = axs[0])
axs[0].set_xlabel("g_ii")
axs[0].set_ylabel("g")
axs[0].scatter(x = [1, g_ii], y = [1, g], color = "red")
axs[0].title.set_text('Static inhibition ')

#plot with inhibitory plasticity 
cs = axs[1].imshow(delta_cor_plast, origin="lower", extent = (g_ii_min, g_ii_max, g_min, g_max), vmin = 1, vmax = 8)
plt.colorbar(cs, ax = axs[1])
axs[1].set_xlabel("g_ii")
axs[1].set_ylabel("g")
axs[1].scatter(x = [1, g_ii], y = [1, g], color = "red")
axs[1].title.set_text('Plastic inhibition ')

#rates_without 
cs = axs[2].imshow(delta_rate, origin="lower", extent = (g_ii_min, g_ii_max, g_min, g_max))
plt.colorbar(cs, ax = axs[2])
axs[2].set_xlabel("g_ii")
axs[2].set_ylabel("g")
axs[2].scatter(x = [1, g_ii], y = [1, g], color = "red")
axs[2].title.set_text('rates')

#inhibitory plasticity needed 
cs = axs[3].imshow(h_is, origin="lower", extent = (g_ii_min, g_ii_max, g_min, g_max))
plt.colorbar(cs, ax = axs[3])
axs[3].set_xlabel("g_ii")
axs[3].set_ylabel("g")
axs[3].scatter(x = [1, g_ii], y = [1, g], color = "red")
axs[3].title.set_text('inhibitory plasticity needed')
